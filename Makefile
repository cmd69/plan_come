...
.PHONY: help up down logs build up-dev down-dev logs-dev build-dev up-pro down-pro build-pro shell-app migrate regen studio setup backup restore
...
up: up-pro ## Alias: producción por defecto
down: down-pro ## Alias: parar producción
logs: logs-pro ## Alias: logs producción
build: build-pro ## Alias: build producción

up-pro: ## Iniciar servicios de PRODUCCIÓN (build local)
	docker compose -f $(PROD_COMPOSE) up -d

down-pro: ## Detener servicios de PRODUCCIÓN
	docker compose -f $(PROD_COMPOSE) down

build-pro: ## Construir imagen de PRODUCCIÓN
	docker compose -f $(PROD_COMPOSE) build

logs-pro: ## Ver logs de PRODUCCIÓN
	docker compose -f $(PROD_COMPOSE) logs -f

deploy: ## Deploy: build + recrear contenedor de PRODUCCIÓN
